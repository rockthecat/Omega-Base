...
public class Logs {
	...
	public static void error(String cls, String error) {
        	Logger log = Logger.getLogger(cls);

                log.error(error);
        }

        public static boolean log(Connection conn, int user, String ip, String hostname, String action, String table, int table_id) {
	try {
        	PreparedStatement ps = conn.prepareStatement(
                "insert into access_log (\"user\", ip, \"action\", \"table\", table_id, date, hostname) "+
	       ...
	       error(Logs.class.getName(), ex.toString());
               return false;
	}
        ...
        public static int getGroup(Connection conn, HttpSession session) {
	try {
        	Statement st = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
                int id_group = 0;

                if (session==null||session.getAttribute("id_group")==null) {
			ResultSet rs = st.executeQuery(
                		"select anonymous_can_use, anonymous_group from \"configuration\" "
                	);
        		...
        		id_group = (Integer)session.getAttribute("id_group");
        	}
        ...
        public static int getPriviledge(Connection conn, int id_group, int id_category) {
	try {
        	Statement st = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
                int level = 0;
                ResultSet  rs = st.executeQuery(
        	" select level_priviledge from group_category "+

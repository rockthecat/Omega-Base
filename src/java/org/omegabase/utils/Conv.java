...
public class Conv {
        ...
        public static SimpleDateFormat TIMESTAMP_DATE = new SimpleDateFormat("yyyy-MM-dd");
	...
	*/
	        URI uri = new URI("http", "//", /*URLEncoder.encode(url, "UTF-8")*/url);
		return URLEncoder.encode(uri.normalize().toASCIIString().substring(8), "UTF-8");
	...
	public static String beanXML(Object []objs) {
	try {
		...
		Logger.getLogger(Conv.class.getName()).severe(ex.getMessage());
	}
		return "<error/>";
	}
	
	
	public static Object[] beanXML(String xml) {
		List<Object> ret = new ArrayList<Object>();
	try {
		XMLDecoder xmld = new XMLDecoder(new ByteArrayInputStream(xml.getBytes()));
		
		while(true) {
			ret.add(xmld.readObject());
		}
	...
	public static String toHTML(int color) {
		...
		s = Integer.toHexString((byte)color).toUpperCase();
		if (s.length()==1) {
	...
	public static int toInt(String html) {
		...
		if (html.length()==3) {
			ret = Integer.parseInt(html.substring(0, 1)+'0', 16);
			ret |= (Integer.parseInt(html.substring(1, 2)+'0', 16)<<8);
			ret |= (Integer.parseInt(html.substring(2, 3)+'0', 16)<<16);
		} else {
			ret = Integer.parseInt(html.substring(0, 2), 16);
			ret |= (Integer.parseInt(html.substring(2, 4), 16)<<8);
			ret |= (Integer.parseInt(html.substring(4, 6), 16)<<16);
		}
